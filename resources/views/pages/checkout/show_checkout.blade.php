									...
									<tr>
										...
										<td colspan="2">
										    ...
										    <li>
										        @foreach(Session::get('coupon') as $key => $cou)
										            @if($cou['coupon_condition']==1)
										                Mã giảm :  <span class="total-amount-text"> {{$cou['coupon_number']}} % </span>
										                <p>
										                    @php 
										            ...
										            @elseif($cou['coupon_condition']==2)
										                Mã giảm :  <span class="total-amount-1">{{number_format($cou['coupon_number'],0,',','.')}}đ</span>
										                <p>
										                    @php 
										    ...
										    <li>	
										        ...
										        Phí vận chuyển :  <span class="total-amount-1">{{number_format(Session::get('fee'),0,',','.')}}đ</span>
										    </li> 
										    @php 
										    ...
										    <li>Tổng còn : 
										        <span class="total-amount-text"> </span>
								...
								@if(Session::get('cart'))
									    <tr>
									        <td>
									            <form method="POST" action="{{ url('/check-coupon') }}">
									                ...
									                <select class="form-control" name="coupon" id="coupon-select" onchange="updateCouponDisplay()">
									                    <option value="">-- Chọn mã giảm giá --</option>
									                    <option value="MAGIAM1000K">MAGIAM1000K - giảm tối đa 1000k cho đơn hàng 15tr - Hạn sử dụng: 11/1/2024</option>
									                    <option value="MAGIAM90k">MAGIAM90k - giảm tối đa 90k cho đơn hàng 500k - Hạn sử dụng: 3/1/2024</option>
									...
									<script>
									    function updateCouponDisplay() {
									        ...
									        if (selectedIndex > 0) { // Đảm bảo người dùng đã chọn một mã giảm giá
									            // Lấy giá trị và thay đổi văn bản hiển thị
									            const selectedOption = selectElement.options[selectedIndex];
									            const couponCode = selectedOption.value;

									            // Chỉ hiển thị mã giảm giá
									            selectedOption.textContent = couponCode;
									        }
									    }
									</script>
						...
						<div class="bill-to">
							    ...
							    <div class="form-one">
							        <form method="POST">
							            ...
							            <!-- <td><a class="btn btn-default check_out" href="{{url('/vnpay-payment')}}">Thanh toán VNPAY</a></td> -->
